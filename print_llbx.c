...
/*   Updated: 2018/11/30 05:09:06 by tduval           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int print_padding(unsigned long long n, int s, t_flags elem)
{
	...
	if (!ft_strchr(elem.options, '-') && elem.width)
	{
		while (elem.width > s + (elem.accuracy != -1 && elem.accuracy > s ? elem.accuracy - s + (ft_strchr(elem.options, '#') ?     2 : 0) : 0))
		{
...
int		print_llbx(va_list ap, t_flags elem)
{
	...
	cp = n;
	if (ft_strchr(elem.options, '#') && (elem.width || n))
		if (elem.accuracy || n)
			i += 2;
	while (cp > 15)
	{
